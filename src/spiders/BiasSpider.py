...
class BiasSpider(BaseSpider):  
  ...
  def genUrl(self,id):
    ...
    url +='k&js=$FUN$((x))&rtntype=4&extend=bias&check=kte&authorityType=fa&$FUN$=$FUN$'
    ...
    http://pdfm2.eastmoney.com/EM_UBG_PDTI_Fast/api/js?id=0000022&TYPE=k&js=fsDataTeacma((x))&rtntype=4&extend=bias&check=kte&authorityType=fa&fsDataTeacma=fsDataTeacma
    '''
...
if __name__ == '__main__':
  ...
  for threadId in xrange(1,threads+1):
    ...
    spider = BiasSpider(subIdList,threadId)
    spider.start()

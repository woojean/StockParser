...
from parsers import MaConvergenceParser
...
买入后持有5天，最高价走（优势衡量）
'''
def traceQ(id,parseDay):
  N = 5
  print id,parseDay
  parser = MaConvergenceParser.MaConvergenceParser(parseDay,id)
...
def traceY(id,parseDay):
  ...
  for day in dayList:
    ...
    if startPrice == 0 or endPrice == 0 or minPrice==0: # 坏数据
      outPrice = 0
      break
